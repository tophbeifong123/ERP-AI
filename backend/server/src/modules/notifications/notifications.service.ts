...
export class NotificationsService {
  ...
  async list(userId: string, filter: { type?: NotificationType; unreadOnly?: boolean }): Promise<Notification[]> {
    const qb = this.notificationRepo.createQueryBuilder('n').where('n.userId = :uid', { uid: userId });
  ...
  async getOne(userId: string, id: string): Promise<Notification> {
    ...
    if (!n) {
      throw new NotFoundException({ message: 'Notification not found', error: 'not_found' });
    }
    return n;
  }
  ...
  async create(userId: string, type: NotificationType, postId?: string, channel = 'email'): Promise<Notification> {
    const n = this.notificationRepo.create({ userId, type, postId: postId ?? null, channel });
